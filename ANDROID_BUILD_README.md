# HeritageFaith V19 Android Build Candidate
...
The public candidate declares only `INTERNET` and `ACCESS_NETWORK_STATE`. Device GPS and microphone recording UI/bridge code were removed from the public candidate rather than adding sensitive permissions. Map handoff still works with coordinates entered by the user; phone/SMS/email use explicit external intents and do not request contacts/SMS/call-log permissions. Android cloud backup is disabled; HeritageFaith uses its explicit export/restore path instead.
