...
m_db=re.search(r"DB_VERSION\s*=\s*(\d+)",db);c("DB v9+",bool(m_db) and int(m_db.group(1))>=9);c("phone tables","phone_intake_runs" in db and "phone_intake_files" in db)
...
m=re.search(r"versionCode\s+(\d+)",grad);c("V24+ code",bool(m) and int(m.group(1))>=240001);c("V24 Home Base lineage",any(x in grad for x in ["homebase-phone-intake","unified-core","full-continuity","import-all","mirror-sweep"]))
